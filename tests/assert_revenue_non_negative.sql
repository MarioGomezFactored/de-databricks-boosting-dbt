...
select 'revenue_by_date'          as model, order_date_id::string                          as grain, total_revenue
...
select 'top_sellers_ranked'       as model, seller_id::string                              as grain, total_revenue
from {{ ref('top_sellers_ranked') }}
where total_revenue < 0

union all

select 'category_performance'     as model, category_id::string                            as grain, total_revenue
from {{ ref('category_performance') }}
where total_revenue < 0

union all

select 'brand_category_sales'     as model, (brand || '|' || category_id::string)         as grain, total_revenue
