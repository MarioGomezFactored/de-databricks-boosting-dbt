...
-- Roll up across currencies; seller attributes are consistent per seller_key
seller_totals as (
    select
        seller_key,
        max(seller_name)        as seller_name,
        max(seller_type)        as seller_type,
        max(seller_country)     as seller_country,
...
from final
order by revenue_rank
>>>>>>> e951ada (feat(models): enriched models with dimension data)
